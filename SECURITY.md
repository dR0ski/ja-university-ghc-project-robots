...
- **Rate limiting:** Flask-Limiter with Redis storage in production. `/register` POST limited per IP and per submitted email (slows enumeration).
...
- **Container:** non-root UID 10001, read-only root FS, all caps dropped, `no-new-privileges`.
